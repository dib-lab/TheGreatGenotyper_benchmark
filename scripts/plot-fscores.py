              ...
              #  print(filename)
                for line in open(filename, 'r'):
                  ...
                  #      print(fields)
                        ...
                        self._size = int(fields[1])+int(fields[4])
...
#        legend_methods = ["TheGreatGenotyper(all)", "TheGreatGenotyper(high-gq)", "TheGreatGenotyper_kmersonly","Beagle",'PanGenie (all)', 'PanGenie (high-gq)'] if regions not in ['repeats', 'nonrep', 'external'] else ["TheGreatGenotyper", "TheGreatGenotyper(high-gq)",  "TheGreatGenotyper_kmersonly", "Beagle",'PanGenie (all)', 'PanGenie (high-gq)', 'Paragraph', 'GATK', 'GraphTyper']
        methods = ["TheGreatGenotyper-kmersOnly", "TheGreatGenotyper-SecondPassHMM" , "TheGreatGenotyper-HMM",'pangenie'] if regions not in ['repeats', 'nonrep', 'external'] else ["TheGreatGenotyper-kmersOnly", "TheGreatGenotyper-SecondPassHMM" , "TheGreatGenotyper-HMM",'pangenie', 'paragraph', 'gatk', 'graphtyper']
        legend_methods = ["TheGreatGenotyper-kmersOnly", "TheGreatGenotyper-SecondPassHMM" , "TheGreatGenotyper-HMM",'PanGenie'] if regions not in ['repeats', 'nonrep', 'external'] else ["TheGreatGenotyper-kmersOnly", "TheGreatGenotyper-SecondPassHMM" , "TheGreatGenotyper-HMM" ,'PanGenie', 'Paragraph', 'GATK', 'GraphTyper']

        qualities = ['0', '0','0','0'] if regions not in ['repeats', 'nonrep', 'external'] else ['0','0', '0','0', '0', '0','0']
...
#       coverages = [str(c) for c in sorted_coverages]
        ...
        for varianttype in variants:#
                ...
                patterns = []
                size=0
                if varianttype not in data:
                        data[varianttype] = {}
                        
                for i, method, quality in zip(range(len(methods)), methods, qualities):
                        if method not in data[varianttype]:
                                data[varianttype][method] = {region:(0,0)}
...
#                               continue
                        ...
                        for run in run_mode:
                                ...
                                filename= folder  + "/evaluation/precision-recall-typable/" + mode + "/" + method + "-" + run + '/coverage-' + str(coverage) + "_" + region + "_" + var + "/qual_" + quality + "/summary.txt"
                                stats = Stats(filename)
                                fscore = stats.get_fscore()
                                data[varianttype][method][region]=(fscore,stats.get_size())
...
#               plt.bar(considered_methods, fscore_all,color=considered_colors, hatch=patterns)
        ...
        plt.tight_layout()
        plt.savefig(outfile)
        print("Category,Method,fscore,size")
        for vtype,d1 in data.items():
                for m,d2 in d1.items():
                        for r,s in d2.items():
                                fscore,size=s
                                if(math.isnan(fscore)): fscore=0
                                print("%s,%s,%.2f,%d"%(vtype,m,fscore,size))
...
#       parser.add_argument('-variants', metavar='VARIANTS', required=True, nargs='+', help='comma separated list of variants.')
        ...
        variants = ["snp", "snp-complex","indel","indel-complex", "large-deletion", "large-insertion", "large-complex" ]

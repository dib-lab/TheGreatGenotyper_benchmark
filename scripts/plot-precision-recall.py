...
# 	methods = ["TheGreatGenotyper", "TheGreatGenotyper" , "NewTheGreatGenotyper",'Beagle','pangenie', 'pangenie'] if regions not in ['repeats', 'nonrep', 'external'] else ["TheGreatGenotyper", "TheGreatGenotyper", "NewTheGreatGenotyper" ,'Beagle','pangenie', 'pangenie', 'paragraph', 'gatk', 'graphtyper']
#	legend_methods = ["TheGreatGenotyper(all)", "TheGreatGenotyper(high-gq)", "TheGreatGenotyper_kmersonly","Beagle",'PanGenie (all)', 'PanGenie (high-gq)'] if regions not in ['repeats', 'nonrep', 'external'] else ["TheGreatGenotyper", "TheGreatGenotyper(high-gq)",  "TheGreatGenotyper_kmersonly", "Beagle",'PanGenie (all)', 'PanGenie (high-gq)', 'Paragraph', 'GATK', 'GraphTyper']

#	qualities = ['0', '200','0','0','0','200'] if regions not in ['repeats', 'nonrep', 'external'] else ['0', '200','0', '0','0', '200', '0', '0','0']
	methods = ["TheGreatGenotyper-kmersOnly", "TheGreatGenotyper-SecondPassHMM" , "TheGreatGenotyper-HMM",'pangenie', 'pangenie'] if regions not in ['repeats', 'nonrep', 'external'] else ["TheGreatGenotyper-kmersOnly", "TheGreatGenotyper-SecondPassHMM" , "TheGreatGenotyper-HMM",'pangenie', 'pangenie', 'paragraph', 'gatk', 'graphtyper']
	legend_methods = ["TheGreatGenotyper-kmersOnly", "TheGreatGenotyper-SecondPassHMM" , "TheGreatGenotyper-HMM",'PanGenie (all)', 'PanGenie (high-gq)'] if regions not in ['repeats', 'nonrep', 'external'] else ["TheGreatGenotyper-kmersOnly", "TheGreatGenotyper-SecondPassHMM" , "TheGreatGenotyper-HMM" ,'PanGenie (all)', 'PanGenie (high-gq)', 'Paragraph', 'GATK', 'GraphTyper']

	qualities = ['0', '0','0','0','200'] if regions not in ['repeats', 'nonrep', 'external'] else ['0','0', '0','0', '200', '0', '0','0']
	sorted_coverages = sorted([int(coverage) for coverage in coverages])
	coverages = [str(c) for c in sorted_coverages]

	markers = ['D', 'x','o', 'P', '*', 'v','+','X','|']
	plot_index = 1
	colors = ['#377eb8', '#ff7f00','#F1E402', '#4daf4a', '#f781bf', '#a65628', '#984ea3', '#808080', "#040000"]
	...
	for varianttype in variants:#
		...
		for i, method, quality in zip(range(len(methods)), methods, qualities):
			...
			if (method in ['paragraph', 'graphtyper']) and (varianttype.startswith('snp') or varianttype.startswith('small') or varianttype.startswith('indel')):
				continue
...
#				continue
			...
			for run in run_mode:
				...
				for j,coverage in enumerate(coverages):
					filename= folder + "/evaluation/precision-recall-" + variantset + "/" + mode + "/" + method + "-" + run + '/coverage-' + str(coverage) + "_" + region + "_" + var + "/qual_" + quality + "/summary.txt"
					...
					if stats.get_size()!=0: size = stats.get_size()
					plt.scatter([recall], [precision], marker='o', color=colors[i], s=float(j*13)+13, zorder=2)
#					print(method,quality ,j,coverage, [recall],[precision])                                        
		...
		plt.title("%s(%d variants)"%(varianttype,size))
...
#		plt.xlim([0,106])
	...
	for i,coverage in enumerate(coverages):
		...
		line = matplotlib.lines.Line2D([],[], color='grey', marker='o', markersize=float(coverage)*0.1+4, linestyle='none', label=label)
		handles.append(line)
